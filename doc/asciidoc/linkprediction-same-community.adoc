...
These communities could be computed by one of the <<algorithms-community>>.
...
If two nodes belong to the same community, there's a greater likelihood that there'll be a relationship between them in future, if there isn't already.
...
By default the community is read from the `community` property, but we can explicitly state the property to read from as well.
