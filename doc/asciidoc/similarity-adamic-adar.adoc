...
[[algorithms-similarity-adamic-adar-usecase]]
...
.The following will return the similarity of Zhen and Arya based only on `FRIENDS` relationships:
[source, cypher]
----
...
[source, cypher]
----
RETURN algo.similarity.adamicAdar(node1:Node, node2:Node, {
