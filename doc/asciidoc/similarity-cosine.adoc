...
[source, cypher]
----
CALL algo.similarity.cosine(userData:Array<Map>>, {
...
[source,cypher]
----
CALL algo.similarity.cosine.stream(userData:List<Map>>, {
...
If the similarity lists are very sparse (i.e. contain a lot of 0s), you can use Cypher statements to project a graph.
