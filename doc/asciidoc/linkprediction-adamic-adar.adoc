...
== Use-cases - when to use the Adamic Adar algorithm

// tag::use-case[]
We can use the Adamic Adar algorithm to work out the closeness between two nodes.
We might then use the computed score as part of a link prediction solution.
// end::use-case[]
...
.The following will return the closeness of Michael and Karin:
[source, cypher]
----
...
.The following will return the Adamic Adar similarity of Michael and Karin based only on `FRIENDS` relationships:
[source, cypher]
----
