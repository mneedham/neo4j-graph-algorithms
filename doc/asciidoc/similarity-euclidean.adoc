...
| `writeBatchSize`            | int     | 1000 | yes      | The batch size to use when storing results.
...
[[algorithms-similarity-euclidean-projection]]
== Cypher projection

If the similarity lists are very sparse (i.e. contain a lot of 0s) they can end up taking up a lot of memory.
For those use cases you can use Cypher statements to project the graph instead.
